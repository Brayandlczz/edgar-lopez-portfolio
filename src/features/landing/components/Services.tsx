    ...
    >
      ...
      <div className="mb-16">
        <motion.p
          ...
          className="f-mono text-[10px] tracking-[0.35em] text-white/45 uppercase mb-4"
        >
          Servicios
        </motion.p>
            ...
            >
              <div className="flex items-center gap-6 md:gap-10">
                ...
                <div className="flex flex-col md:flex-row md:items-baseline gap-1 md:gap-5">
                  ...
                  <span className="f-mono text-[9px] text-white/40 tracking-wide hidden md:inline">
                    {s.summary}
                  </span>
                </div>
              </div>
              ...
              <motion.span
                ...
                aria-hidden
                className="text-white/45 text-xl font-light shrink-0 ml-4 leading-none group-hover:text-white/50 transition-colors duration-200"
              >
                +
              </motion.span>
            </button>
              ...
              {open === s.id && (
                ...
                >
                  <div className="pl-11 md:pl-16 pr-4 pb-10 grid md:grid-cols-[1.2fr_1fr] gap-8 md:gap-16 items-start">
                    ...
                    {/* Descripción */}
                    <p className="text-white/60 text-sm leading-[1.9] text-justify">
                      {s.what}
                    </p>

                    {/* Items */}
                    <div>
                      <p className="f-mono text-[8px] tracking-[0.3em] text-white/60 uppercase mb-4">
                        Incluye:
                      </p>
                      <ul className="space-y-2.5" aria-label={`Qué incluye ${s.title}`}>
                        {s.items.map((it) => (
                          <li key={it} className="flex items-center gap-3">
                            <span aria-hidden className="w-3 h-px bg-white/40 shrink-0" />
                            <span className="f-mono text-[10px] text-white/60 tracking-wide">{it}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
